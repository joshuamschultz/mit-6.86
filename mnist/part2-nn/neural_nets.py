...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        hidden_layer_error = np.multiply(self.hidden_to_output_weights.T, output_derivative_vec(activated_output)) * output_layer_error

        # f(activation)  x backprop error
        bias_gradients = np.multiply(hidden_layer_error, ReLU_derivative_vec(hidden_layer_weighted_input)) #ReLU_derivative_vec(hidden_layer_weighted_input.T).dot(hidden_layer_error)
